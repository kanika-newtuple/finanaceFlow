...
class Logger:
    ...
    def __init__(self):
        self.ENABLE_OTEL_COLLECTOR = os.getenv("ENABLE_OTEL_COLLECTOR", False)
        self.ENABLE_JSON_LOGGER = os.getenv("ENABLE_JSON_LOGGER", True)
    ...
    def enable_otel(self):
        ...
        http_exporter = OTLPHTTPLogExporter(endpoint=f"{OTEL_AGENT_HOSTNAME}:{OTEL_AGENT_PORT}/v1/logs")
        grpc_exporter = OTLPGRPCLogExporter(endpoint=f"{OTEL_AGENT_HOSTNAME}:{OTEL_AGENT_PORT}", insecure=True, compression=Compression.Gzip.value)
        ...
        self.shell_handler.setFormatter(CustomJsonFormatter(self.otel_fmt_file))
